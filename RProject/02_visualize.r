...
switzerland <- read_sf("natural_earth.gpkg", "switzerland")

world_countries <- read_sf("natural_earth.gpkg", "world")
...
write_sf(my_circles_filter, "toy_data.gpkg","my_circles_filter",delete_layer = FALSE)
...
ortho <- \(lon = 2, lat = 45){paste0('+proj=ortho +lat_0=', lat, ' +lon_0=', lon, ' +x_0=0 +y_0=0 +a=6371000 +b=6371000 +units=m +no_defs')}

get_circle <- \(lon, lat)st_point(x = c(0,0)) %>% st_buffer(dist = 6371000) %>% st_sfc(crs = ortho(lon,lat))
...
equator <- lon_lat2line(x1, 0)
 
greenwich1 <- lon_lat2line(0, y2)
greenwich2 <- lon_lat2line(180, y2)

greenwich <- c(greenwich1, greenwich2)  
...
## WGS84
ggplot() +
  geom_sf(data = get_circle(30,35), fill = "#68838B") +
  geom_sf(data = world_countries, fill = "darkgrey",color = "lightgrey") +
  geom_sf(data = equator) +
  geom_sf(data = greenwich) +
  coord_sf(crs = ortho(30,35)) +
  theme(panel.background = element_rect(fill="transparent"),
        plot.background = element_rect(fill="transparent", color=NA),
        panel.ontop = FALSE) 
...
ggplot() +
  geom_sf(data = get_circle(2,45), fill = "#68838B") +
  geom_sf(data = world_countries, fill = "darkgrey",color = "lightgrey") +
...
lakes_ch <- read_sf("natural_earth.gpkg", "lakes_ch")
